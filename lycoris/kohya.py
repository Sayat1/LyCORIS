...
class LycorisNetworkKohya(LycorisNetwork):
    ...
    def prepare_optimizer_params(
        ...
        def assemble_params(loras, lr, ratio):
            ...
            for lora in loras:
                for name, param in lora.named_parameters():
                    if ratio is not None and ("resnets" in lora.lora_name or "conv" in lora.lora_name):#("lora_up" in name or "lokr_w1_a" in name or "lokr_w2_a" in name):
        ...
        if self.text_encoder_loras:
            ...
            for i, (te_loras,t_lr) in enumerate(zip([te1_loras,te2_loras],text_encoder_lrs,strict=True)):
                ...
                lr_descriptions.extend(
                    [f"textencoder {i+1}" + (" " + d if d else "") for d in descriptions]
                )

                print(f"TE{i+1} param created with {[len(param['params']) for param in params]}")
        ...
        if self.unet_loras:
            ...
            print(f"UNET param created with {[len(param['params']) for param in params]}")
